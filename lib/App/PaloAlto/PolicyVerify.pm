...
sub new {
    ...
    if ((my $nfields = @{ $obj{fields} }) != 5) {
        die "'fields' argument has $nfields comma-separated values; needs to be 5 - e.g. '2,4,7,8,9'";
    }
...
sub logfile {
    ...
    open($self->{fh}, '<:encoding(utf8)', $filepath) or die "Could not open file '$filepath'";
...
sub fields {
    ...
    $self->{fields} = [ @column_numbers{qw(src_ip dst_ip src_port dst_port protocol)} ];
...
sub run { 
    ...
    while ( my $row = $self->{csv}->getline( $self->{fh} ) ) {
        ...
        if ($num_fields != 5) {
            die "Only $num_fields fields extracted based on the separating character. We expect 5";
        }

        # Set up the cache key and return an entry if it exists
        # Note that we don't cache based on source port.
        $flow_cache_key = join(':', @flow_info[SRC_IP, DST_IP, DST_PORT, PROTO]);

        # Pull the result out of cache, or we test
        # the security policy
        my $result = $run_cache{flow_cache_key} || $self->_test_sec_policy(@flow_info);
        next unless $result;

        # Add the result to the cache if needed.
        $run_cache{$flow_cache_key} //= $result;

        say $result->rulename.','.$result->action.','.$result->index.',';
    }
